...
// the returned node.oneMoreTime == true, it will be walked over again.
...
function timed(func, name) {
    ...
    var ret = func();
    util.log(new Array(timed_level - 1).join('  ') + (new Date().getTime() - time) / 1000 + ' seconds [' + name + ']');
    timed_level--;
    return ret;
}

(function() {
    function timerize(name, funcName, obj) {
        var f = obj[funcName];
        obj[funcName] = function() {
            var args = [].slice.call(arguments);
            return timed(function() { return f.apply(obj, args)}, name);
        }
    }
    
    timerize('================= TOTAL', 'init', readability);
    timerize('prepDocument', 'prepDocument', readability);
    timerize('prepArticle', 'prepArticle', readability);
    timerize('grabArticle', 'grabArticle', readability);
...
function start(w, cb) {
    ...
    try {
        ...
        //throw e;
        console.log('Error', e, e.message, e.stack);
    }
...
exports.parse = function parse(html, url, callback) {
    //util.debug(html);
